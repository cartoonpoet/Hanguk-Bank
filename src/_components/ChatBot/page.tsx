...
const ChatBot = ({ style }: { style?: any }) => {
  const router = useRouter()
  return (
    <main style={style} className='flex flex-col h-dvh font-pretendard'>
      <NavBar
        leftControl='icon'
        rightControl='icon'
        title={
          <div>
            <span className='text-blue-500'>챗봇</span> 상담
          </div>
        }
      />
      <div className='bg-white h-4' />
      <section className='flex flex-row px-5 py-2.5 justify-between bg-Brand-On-Tertiary text-white text-base items-center'>
        <div>챗봇과 대화중</div>
        <button
          className='border px-4 py-2 rounded-3xl'
          onClick={() => router.replace(URL.home)}
        >
          상담 종료
        </button>
      </section>
      <section className='flex flex-col bg-Brand-Secondary flex-1'>
        <div className='p-5 text-center text-Neutral-Label text-sm font-medium'>
          2025년 1월 6일 화요일
        </div>
        <div className='flex flex-col px-5 gap-1'>
          <div className='flex flex-row items-center gap-2'>
            ...
            <div className='text-sm font-medium text-Neutral-Label'>
              오전 11:18
            </div>
          </div>
          ...
          <nav className='flex flex-row gap-1 mt-1'>
            {buttons.map((button) => (
              <button
                key={button}
                className='text-sm font-semibold bg-white px-4 py-2 rounded-3xl text-Brand-Default'
              >
                {button}
              </button>
            ))}
